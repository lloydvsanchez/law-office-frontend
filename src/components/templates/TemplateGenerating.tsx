...
// Communicates clearly that work is happening — not a broken page.
...
// Rotating status messages so the wait feels active, not frozen
...
const TemplateGenerating: React.FC<TemplateGeneratingProps> = ({
  ...
  return (
    <div className="w-full max-w-xl mx-auto flex flex-col items-center text-center py-12">
      ...
      {query && (
        <p className="font-sans text-sm text-[#C9A84C] mb-6">
          "{query}"
        </p>
      )}
      ...
      <p
        key={message} // triggers re-render animation when message changes
        className="font-sans text-sm text-[#5C5C5C] leading-relaxed max-w-sm
          transition-opacity duration-500"
      >
        {message}
      </p>

      {/* Subtle poll count indicator */}
